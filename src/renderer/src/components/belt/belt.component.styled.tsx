...
import { filterProps } from '@renderer/styles/filter.props.helper'
...
export const getRankBarColor = (belt: BeltColor): string | null => {
  ...
  if (/^(grey|yellow|orange|green)/.test(belt)) return BJJ_BELT_HEX[BeltColor.BLACK]
  return null
}
...
export const BeltBand = filterProps<{ color: string; scale: number }>('div')`
  ...
  align-items: center;
  box-shadow: 0 ${(props) => 1 * props.scale}px ${(props) => 3 * props.scale}px rgba(0, 0, 0, 0.2);
`

export const StripeContainer = filterProps<{ color: string; scale: number }>('div')`
  background-color: ${(props) => props.color};
  display: flex;
...
export const OverlayStripe = filterProps<{ color: string; scale: number }>('div')`
  position: absolute;
  top: 50%;
  left: 0;
...
export const BlackBeltStripe = filterProps<{ color: string; scale: number }>('div')`
  background-color: ${(props) => props.color};
...
export const RedBeltStripe = filterProps<{ color: string; scale: number }>('div')`
  background-color: ${(props) => props.color};
